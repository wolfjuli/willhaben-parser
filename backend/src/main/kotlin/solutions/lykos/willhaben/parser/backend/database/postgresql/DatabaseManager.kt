...
class DatabaseManager(
    ...
    private val patchesUrl: URL = URL(baseUrl.toExternalForm() +  "/patches")
    private val schemaUrl: URL = URL(baseUrl.toExternalForm() +  "/schema")
    ...
    private fun runSchema(
        ...
        return runPsql(
            ...
            inputStreamOf(
                Supplier { "BEGIN;\n\n".byteInputStream() },
                Supplier {
                    (File(schemaUrl.toURI()).listFiles() ?: emptyArray())
                        .sortedBy { it.name }
                        .joinToString("\n\n") { it.readText() }
                        .byteInputStream()
                },
