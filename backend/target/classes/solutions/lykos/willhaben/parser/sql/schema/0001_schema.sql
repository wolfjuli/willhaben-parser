...
CREATE TYPE data_type AS ENUM ('TEXT', 'IMAGE', 'LINK');
...
DECLARE
    ...
    sql TEXT ;
BEGIN
    sql := 'SELECT to_jsonb(' || _function || ') from (values(''' || replace(coalesce(attribute, ''), '''', '\''') || ''', ''' ||
           listing::TEXT ||
           '''::JSONB)) r(val, row)';
...
CREATE OR REPLACE FUNCTION run_function(function_id SMALLINT, attribute TEXT, listing JSONB) RETURNS JSONB
    LANGUAGE plpgsql
AS
$$
BEGIN
...
DROP MATERIALIZED VIEW  IF EXISTS normalized_listings;
CREATE MATERIALIZED VIEW  normalized_listings(listing_id, willhaben_id, listing, md5) AS
WITH RECURSIVE
...
CREATE OR REPLACE FUNCTION get_id() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
DECLARE
...
CREATE OR REPLACE FUNCTION changed_listing_attributes() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION change_listing_custom_attributes() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION changed_normalized_listings() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION changed_user_defined_attributes() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION changed_script_functions() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION changed_custom_attributes() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION changed_functions() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION toggle_triggers(action TEXT, namespace TEXT DEFAULT 'willhaben'::TEXT) RETURNS VOID
    LANGUAGE plpgsql
AS
$$
DECLARE
